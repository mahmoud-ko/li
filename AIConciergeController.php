...
class AIConciergeController {
    ...
    public function __construct() {
        ...
        // إذا لم يوجد، حاول القراءة من ملف .env (على الخادم فقط)
    ...
    private function callGroqAPI(string $message, array $matches): ?string {
        ...
        $hotelsList = "";
        foreach ($matches as $h) $hotelsList .= "- {$h['name']} in {$h['city']}: \${$h['price']}/night, {$h['stars']}★\n";
    ...
    private function generateLocalResponse(array $parsed, array $matches): string {
        ...
        if (count($matches) > 1) $reply .= "Another great option is {$matches[1]['name']} from \${$matches[1]['price']}/night. Would you like to check availability?";
        return $reply;
    }
    ...
    public function trainFromConversations(): array {
        ...
        $cityStats = [];
        foreach ($insights as $i) $cityStats[$i['extracted_city']] = ($cityStats[$i['extracted_city']]??0) + $i['count'];
        return ['top_destinations'=>$cityStats, 'total_conversations'=>(int)$this->db->query("SELECT COUNT(*) FROM ai_conversations")->fetchColumn(), 'insights'=>$insights];
    }
}
?>
